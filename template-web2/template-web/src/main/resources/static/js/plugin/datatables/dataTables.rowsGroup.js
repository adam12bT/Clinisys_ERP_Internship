...
(function ($) {
     ...
     */
    let RowsGroup = function (dt, columnsForGrouping) {
        this.table = dt.table();
    ...
    RowsGroup.prototype = {
        ...
        _getOrderWithGroupColumns: function (order, groupedColumnsOrderDir) {
            ...
            let self = this;
            let groupedColumnsIndexes = this.columnsForGrouping.map(function (columnSelector) {
                return self.table.column(columnSelector).index()
            });
            let groupedColumnsKnownOrder = order.filter(function (columnOrder) {
                return groupedColumnsIndexes.indexOf(columnOrder[0]) >= 0
            });
            let nongroupedColumnsOrder = order.filter(function (columnOrder) {
                return groupedColumnsIndexes.indexOf(columnOrder[0]) < 0
            });
            let groupedColumnsKnownOrderIndexes = groupedColumnsKnownOrder.map(function (columnOrder) {
                return columnOrder[0]
            });
            let groupedColumnsOrder = groupedColumnsIndexes.map(function (iColumn) {
                let iInOrderIndexes = groupedColumnsKnownOrderIndexes.indexOf(iColumn);
        ...
        _getInjectedMonoSelectWorkaround: function (order) {
            if (order.length === 1) {
                ...
                let orderingColumn = order[0][0];
                let previousOrder = this.order.map(function (val) {
                    return val[0]
                });
                let iColumn = previousOrder.indexOf(orderingColumn);
                if (iColumn >= 0) {
        ...
        _mergeCells: function () {
            let columnsIndexes = this.table.columns(this.columnsForGrouping, ShowedDataSelectorModifier).indexes().toArray();
            let showedRowsCount = this.table.rows(ShowedDataSelectorModifier)[0].length;
        ...
        _mergeColumn: function (iStartRow, iFinishRow, columnsIndexes) {
            let columnsIndexesCopy = columnsIndexes.slice();
            currentColumn = columnsIndexesCopy.shift();
            currentColumn = this.table.column(currentColumn, ShowedDataSelectorModifier);

            let columnNodes = currentColumn.nodes();
            let columnValues = currentColumn.data();
    ...
    $(document).on('init.dt', function (e, settings) {
        ...
        if (settings.oInit.rowsGroup ||
            ...
            options = settings.oInit.rowsGroup ?
                ...
                $.fn.dataTable.defaults.rowsGroup;
            let rowsGroup = new RowsGroup(api, options);
